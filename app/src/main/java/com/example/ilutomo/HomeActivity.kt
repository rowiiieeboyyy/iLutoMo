...
class HomeActivity : AppCompatActivity() {
    ...
    private val businessInventory = mutableListOf<InventoryItem>()
    ...
    private fun loadData() {
        // Load Inventory first, then Library, then User Profile, then Recipes
            ...
            override fun onDataChange(bizSnap: DataSnapshot) {
                ...
                for (biz in bizSnap.children) {
                    for (inv in biz.child("inventory").children) {
                        inv.getValue(InventoryItem::class.java)?.let { 
                            it.id = inv.key ?: ""
                            businessInventory.add(it) 
                        }
                    }
                }
                ...
                for (child in s.children) {
                    if (child.key?.startsWith("recipe_") == true) {
                        try {
                            ...
                            // GREEDY CALCULATION: Price based on CHEAPEST available inventory items
                            ...
                            r.ingredients?.forEach { (name, amt) ->
                                ...
                                if (lib != null) {
                                    ...
                                    // 2. Price: Find cheapest item in store inventory for this ingredient
                                    val cheapestStoreItem = businessInventory
                                        .filter { it.ingredient.equals(name, true) || it.ingredientTag.equals(name, true) }
                                        .minByOrNull { it.price }
                                    
                                    val unitPrice = cheapestStoreItem?.price ?: toFilterDouble(lib.child("price").value, 0.0)
                                    val cost = if (cheapestStoreItem != null) unitPrice else (unitPrice * (qty / 100.0))
    ...
    private fun setupRecyclerView() {
        recipeAdapter = RecipeAdapter(filteredList) { r ->
            ...
            // GREEDY: When adding to pantry, automatically select the CHEAPEST items to fit the initial view
    ...
    private fun showIngredientRecommendationDialog() {
        ...
        currentFridgeIngredients.forEach { addChipToUI(it, chipGroup) }
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Recommend") { _, _ ->
                if (currentFridgeIngredients.isNotEmpty()) runRecommendationAlgorithm(currentFridgeIngredients)
            }
